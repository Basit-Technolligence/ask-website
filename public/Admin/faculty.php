...
while($row=mysqli_fetch_assoc($result))
{
 ...
 if (isset($_POST['updateFaculty'])){
	...
	move_uploaded_file($_FILES["photo"]["tmp_name"],'uploads/faculty/'.$picname);
	if($picname==null)
	{
...
$query="update faculty set name='$newFacultyName',long_description='$newFacultyLong',picture='$newFacultyPicname',short_description='$newFacultyShort' where id=$id";
...
location='faculty.php';
 ...
 if (isset($_POST['faculty'])){
	 ...
	 $picname=$_FILES["photo"]["name"];
	move_uploaded_file($_FILES["photo"]["tmp_name"],'uploads/faculty/'.$picname);
$query="INSERT INTO faculty (name,long_description,short_description,picture) values ('$name','$long','$short','$picname')";
$check=mysqli_query($conn,$query);
if($check==true)
{
echo "<script type='text/javascript'>
...
while($row=mysqli_fetch_assoc($result))
{
 ...
 <td>".$row["name"]."</td>
		...
		<td><img class='img-responsive' alt='picture not found' src='uploads/faculty/".$row["picture"]."' height='50px' width='100px'/></td>
		<td><a href='faculty.php?id=".$row['id']."'><img src='uploads/edit.png' alt='picture not found' class='float-right' /></a><a href='deletefaculty.php?id=".$row['id']."'><img src='uploads/delete.png' alt='picture not found' class='float-right' /></a> </td>
      </tr>";
}
?>
   </tbody>
